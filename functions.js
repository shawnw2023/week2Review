...
//CallBacks// 
let goodGreeting = function(person) {
    console.log("WOW, it's been forever I have missed seeing you around " + person)
}

let poorGreeting = (person) => {
...
greeting("Chris", goodGreeting)
greeting("Jessy", poorGreeting) 

//Other callBack//

let interact = function(animal, callback) {
    callback(animal)
} 

let petAnimal = (animal) => {
    console.log(`You decided to pet the ${animal}.`)
}

let sayHello = (animal) => {
    console.log(`you say hello to the ${animal} from a distance.`)
}

function avoidAnimal(animal) {
    console.log(`You ran away from the ${animal} because he scary...`)
}

interact("Dog", petAnimal)
interact("Cat", petAnimal)
interact("Rabbit", sayHello)
interact("Bear", avoidAnimal)
