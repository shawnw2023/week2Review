...
// const map2 = map1.map(function(num) {
//     return num - 1
// })
console.log(map2)
...
let map3 = map1.map((num) => num.toString())
console.log(map3)
...
let map5 = map4.map((person) => "President " + person.name)
console.log(map5)

// creating a new key
let map5 = map4.map((person) => {
    person.title = "president " + person.name
    return person
})
...
let newArr = arrNum.filter((num) => num % 2 !== 0) // odd number because !== does not. === equals
console.log(newArr)
...
let superOdd = arrNum.filter((num, index) => index % 2 !== 0 && num % 2 !== 0)
console.log(superOdd)
...
let lowerFood = arrString.filter((food) => food.includes('a')) 
console.log(lowerFood)
...
//  modification).
map1.forEach((num, index, arr) => arr[index] = num - 1)
...
const sumOf = reduce1.reduce((acc, num) => acc + num)
console.log(sumOf)
